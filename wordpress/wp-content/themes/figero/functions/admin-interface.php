...
function figero_optionsframework_add_admin() {
    global $query_string;

    $themename = figero_get_option('of_themename');
    $shortname = figero_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = figero_get_option('of_template');
            figero_reset_options($options, 'optionsframework');
    ...
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'figero_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'figero_load_only');
}

add_action('admin_menu', 'figero_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function figero_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        figero_delete_option('of_options');
    }
}
...
function figero_optionsframework_options_page() {
    $options = figero_get_option('of_template');
    $themename = figero_get_option('of_themename');
    ?>
    <div class="trail-notify">
        <h1><?php _e('Get Figero Theme PRO!', 'figero'); ?></h1>
        <p><?php _e('You are Using the FREE Version of Figero Theme. Upgrade to Pro Version for extra features, Home Page Slider, Contact Page Template, Gallery Page, Multiple Color Skins, Premium Support and much more.', 'figero'); ?> </p>
        <a href="http://www.inkthemes.com/wp-themes/figero-wordpress-theme/" target="blank"><?php _e('Upgrade to Figero Theme PRO &rarr;', 'figero'); ?></a>

    </div>
    <div class="wrap" id="of_container">
        <div id="of-popup-save" class="of-save-popup">
            <div class="of-save-save"><?php _e('Options Updated', 'figero'); ?></div>
        </div>
        <div id="of-popup-reset" class="of-save-popup">
            <div class="of-save-reset"><?php _e('Options Reset', 'figero'); ?></div>
        </div>
        <form action="" enctype="multipart/form-data" id="ofform">
            <div id="header">
                <div class="logo">
                    <h2><?php echo $themename; ?> <?php _e('Options', 'figero'); ?></h2>
                </div>
                <a href="http://www.actthemes.com" target="_new">
            ...
            $return = figero_optionsframework_machine($options);
            ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
...
function figero_admin_style() {
    wp_enqueue_style('inkthemes-adminstyle', get_template_directory_uri() . '/functions/admin-style.css', false);
}

add_action('admin_print_styles', 'figero_admin_style');

function figero_load_only() {
    ...
    function of_admin_head() {
        ...
        // COLOR Picker 
        wp_enqueue_style('actthemes-colorpicker', get_stylesheet_directory_uri() . '/functions/css/colorpicker.css', false);
        wp_enqueue_script('actthemes-colorpicker', get_stylesheet_directory_uri() . '/functions/js/colorpicker.js', array('jquery'));
        ?>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = figero_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = figero_get_option($option_id);
                }
                ?>
        ...
        <?php
        //AJAX Upload
        wp_enqueue_script('actthemes-ajaxupload', get_stylesheet_directory_uri() . '/functions/js/ajaxupload.js', array('jquery'));
        ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - figero_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'figero_ajax_callback');

function figero_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        figero_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = figero_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            figero_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    figero_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    figero_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            figero_update_option($multicheck_id, 'false');
                        } else {
                            figero_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    figero_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    figero_update_option($id, $border_array);
...
function figero_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = figero_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'textarea':
                ...
                break;
            case "radio":
                $select_value = figero_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= figero_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= figero_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = figero_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = figero_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = figero_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = figero_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - figero_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function figero_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = figero_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (figero_get_option($id) != "") {
            $val = figero_get_option($id);
        }
