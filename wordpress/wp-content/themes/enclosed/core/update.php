    ...
    if($update_values['filesystem'] != false){ ?>   
        <div class="wrap">
            ...
            <form method="post"  enctype="multipart/form-data" id="cpoform" action="<?php echo $update_values['url']; ?>">
                ...
                <h2><?php _e('A new version of the CPO Core is available.', 'cpocore'); ?></h2>
                <p><?php _e('Updating the CPO Core will download and install the latest version into your theme installation.', 'cpocore'); ?></p>
                <p><?php _e('It is advised that you first create a backup of your theme files before executing the update process.', 'cpocore'); ?></p>
                <h4><?php _e('Current version is', 'cpocore'); ?> <strong><?php echo $update_values['version_current']; ?></strong></h4>	
                <h4><?php _e('Latest version is', 'cpocore'); ?> <strong><?php echo $update_values['version_latest']; ?></strong></h4>
                <input type="submit" class="button-primary" value="<?php _e('Update Core', 'cpocore'); ?>" />
                
                <?php else: ?>
                
                <h3><?php _e('You have the latest version of CPO Core', 'cpocore'); ?></h3>
...
function cpotheme_update_print_advice() {
    if (isset($_REQUEST['page'])) {
        ...
        if ($_page != 'cpotheme_update')
            echo '<div class="updated"><p>' . __('A new version of CPO Core is available. Go to the update page.', 'cpocore') . '</p></div>';
    } else {
        echo '<div class="updated"><p>' . __('A new version of CPO Core is available. Go to the update page.', 'cpocore') . '</p></div>';
    }
}
